...
func (c *StatCache) get(name string, depth int) *cacheEntry {
	...
	// cached, we can infer that the child does is notFound.
...
func (c *StatCache) set(name string, depth int, ce *cacheEntry) {
	...
	if depth == 1 {
		...
		case http.StatusNotFound:
			// Record notFound as the self entry as well.
			self = ce
		case http.StatusMultiStatus:
			...
			if err != nil {
				...
				log.Printf("statcache.set: %s", err)
				return
			}
...
// is somewhere between difficult and impossible.
